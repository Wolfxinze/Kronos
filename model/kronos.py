...
def auto_regressive_inference(tokenizer, model, x, x_stamp, y_stamp, max_context, pred_len, clip=5, T=1.0, top_k=0, top_p=0.99, sample_count=5, verbose=False, reduce="mean"):
    with torch.no_grad():
        ...
        preds = z.cpu().numpy()
        if reduce == "mean":
            preds = np.mean(preds, axis=1)  # (B, T, F)
        # reduce == "none" -> keep every sampled path: (B, sample_count, T, F)
...
class KronosPredictor:
    ...
    def generate(self, x, x_stamp, y_stamp, pred_len, T, top_k, top_p, sample_count, verbose, reduce="mean"):
        ...
        preds = auto_regressive_inference(self.tokenizer, self.model, x_tensor, x_stamp_tensor, y_stamp_tensor, self.max_context, pred_len,
                                          self.clip, T, top_k, top_p, sample_count, verbose, reduce=reduce)
        if reduce == "none":
            preds = preds[:, :, -pred_len:, :]  # (B, sample_count, pred_len, F)
        else:
            preds = preds[:, -pred_len:, :]     # (B, pred_len, F)
        return preds
    ...
    def predict_paths(self, df, x_timestamp, y_timestamp, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=30, verbose=False):
        """Return EVERY sampled future path (no averaging across samples).

        This is the kronos-trader extension to upstream Kronos: the per-sample
        distribution of future OHLCV is what lets us compute a model-implied
        win probability (fraction of paths clearing a return threshold).

        Returns:
            np.ndarray of shape (sample_count, pred_len, n_features) in raw price
            units, columns ordered ['open','high','low','close','volume','amount'].
        """
        if not isinstance(df, pd.DataFrame):
            raise ValueError("Input must be a pandas DataFrame.")
        if not all(col in df.columns for col in self.price_cols):
        ...
        preds = self.generate(x, x_stamp, y_stamp, pred_len, T, top_k, top_p, sample_count, verbose, reduce="none")
        preds = preds[0]  # (sample_count, pred_len, F)
        preds = preds * (x_std + 1e-5) + x_mean
        return preds
    ...
    def predict_paths_batch(self, df_list, x_timestamp_list, y_timestamp_list, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=30, verbose=False):
        """Batched per-sample forecasting (kronos-trader extension).

        Like ``predict_batch`` but returns EVERY sampled path for every series instead of the
        per-series mean. This lets a backtest evaluate many lookback windows in a single
        autoregressive pass, which is what makes large-scale win-rate evaluation tractable.

        All series must share the same historical length and pred_len.

        Returns:
            np.ndarray of shape (num_series, sample_count, pred_len, n_features) in raw price units.
        """
        if not (len(df_list) == len(x_timestamp_list) == len(y_timestamp_list)):
            raise ValueError("df_list, x_timestamp_list, y_timestamp_list must have equal lengths.")
        num_series = len(df_list)

        x_list, x_stamp_list, y_stamp_list, means, stds, seq_lens, y_lens = [], [], [], [], [], [], []
        for i in range(num_series):
            df = df_list[i].copy()
            if not all(col in df.columns for col in self.price_cols):
                raise ValueError(f"DataFrame at index {i} is missing price columns {self.price_cols}.")
            ...
            if y_stamp.shape[0] != pred_len:
                raise ValueError(f"y_timestamp[{i}] length {y_stamp.shape[0]} != pred_len {pred_len}.")
        ...
        if len(set(seq_lens)) != 1 or len(set(y_lens)) != 1:
            raise ValueError(f"Batch requires uniform lengths; got seq={seq_lens}, y={y_lens}")
        ...
        preds = self.generate(x_batch, x_stamp_batch, y_stamp_batch, pred_len, T, top_k, top_p,
                              sample_count, verbose, reduce="none")  # (B, sample_count, pred_len, F)
